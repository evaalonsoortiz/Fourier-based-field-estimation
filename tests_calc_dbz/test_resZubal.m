...
%%
%% Parameters
dbz_path = 'tests_calc_dbz/figures/buffers_Zubal/zubal_xyzbuffers/dbz_ZubalTest_buffer_trunc'
b0 = 3; % [T]
...
n = 40;
z_dims = 2 * ceil(linspace(0, 128, n)); % Be sure to have even and integers values
xy_dims = 2 * ceil(linspace(0, 256, n)); % Be sure to have even and integers values
...
for zi = 1:n 
    ...
    dBz_obj.save(sprintf('%s_xy%u_z%u', dbz_path, xy_dims(zi), z_dims(zi)));
...
title('Quadratique errors between iterations while the dimension of the xyz-buffer increases')

figure; plot(z_dims, glob_diffs, '.-');
xlabel('Pixels added in the z direction')
ylabel('Quadratique error (ppm^2)')
title('Quadratique errors with the last best result (for buffer of x256 y256 z128) while the dimension of the xyz-buffers increases')
